...
class WeatherAPI:
    ...
    def get_all_cities_weather_fast(
        self, cities: Dict[str, str]
    ) -> List[Dict[str, Any]]:
        """
        快速获取所有城市的天气数据（优化版本，减少API调用）
        ...
        for i in range(0, len(location_ids), batch_size):
            ...
            url = f"https://{self.api_domain}/v7/weather/3d?location={batch_str}&key={self.api_key}"
            try:
                response = requests.get(url, headers=self.headers, timeout=15)
                data = response.json()
                if data.get("code") == "200":
                    ...
                    for item in daily_data:
                        ...
                        if loc not in location_forecasts:
                            location_forecasts[loc] = []
                        location_forecasts[loc].append(item)
            except Exception:
                pass
